...
@TeleOp(name = "Webcam Tests", group = "15118")
...
//                }
                ...
                if(gamepad1.a)
                {
                    ...
                    int placed = objectPlacement(frameQueue.poll(), 0);
                    if(placed == -1)
                    {
                        telemetry.addLine("ERROR!");
                        telemetry.update();
                        throw new RuntimeException("ERROR PROCESSING FRAME");
                    } else
                    {
                        if(placed == 0)
                        {
                        ...
                        telemetry.addLine("TOOK " + Double.toString(getRuntime() - startTime));
                        telemetry.update();
                    }
                }
     ...
     * -1 = ERROR
     */
    private int objectPlacement(Bitmap frame, int loopCount)
    {
        if(loopCount >= 1000)
        {
            return -1;
        ...
        if(frame == null)
        {
            return objectPlacement(frame, loopCount + 1); //Recurs if frame is null
        }
